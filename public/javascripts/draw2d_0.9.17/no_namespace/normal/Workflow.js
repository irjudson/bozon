...
Workflow.prototype.clear=function()
{
   ...
   {
     new CommandDelete(tmpLines.get(i)).execute();
   }

   var tmpFigures =  document.getFigures().clone();
   for(var i=0;i<tmpFigures.getSize();i++)
   {
     new CommandDelete(tmpFigures.get(i)).execute();
   }
 ...
 **/
Workflow.prototype.addSelectionListener=function(/*:WindowFigure*/ w)
{
  if(w!=null)
 ...
 **/
Workflow.prototype.removeSelectionListener=function(/*:WindowFigure*/ w )
{
 ...
 * @since 0.9.15
 **/
Workflow.prototype.getLine=function(/*:String*/ id)
{
  var count = this.lines.getSize();
  for(var i=0; i<count;i++)
  {
...
/**
 * @param {Port} p The port to remove from the workflow Canvas.
 * @private
 **/
